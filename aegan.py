...
class AeGAN:
    ...
    def load_generator(self, pretrained_dir=None):
        if pretrained_dir is not None:
            ...
            path = f'{self.params["root_dir"]}/generator.dat'
        self.logger.info("load: "+path)
    ...
    def train_ae(self, dataset, epochs=800):
        ...
        for i in range(epochs):
            ...
            tot_loss/=tot
            if i % 5 == 0:
                self.logger.info(f"Epoch:{i+1} time={time.time()-t1}\tforce={force}\tcon_loss={con_loss/tot}\tdist_loss{dis_loss/tot}\tmiss_loss1={miss_loss1/tot}\tmiss_loss2={miss_loss2/tot}")
                    
            if i % 100 == 99:      
                torch.save(self.ae.state_dict(), f'{self.params["root_dir"]}/ae_{i}.dat') 
